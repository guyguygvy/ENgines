...
float4 PS(VS_OUTPUT input) : SV_Target
{
    ...
    else if(mode == 9)  // infrared
    {
       ...
       float heatValue = saturate((intensity - param0) * param1);
       float3 thermalColor;
       if (heatValue < 0.5)
       {
           thermalColor = lerp(float3(0.0, 0.0, 1.0), float3(1.0, 0.0, 0.0), heatValue * 2.0);
       }
       else
       {
           thermalColor = lerp(float3(1.0, 0.0, 0.0), float3(1.0, 1.0, 0.0), (heatValue - 0.5) * 2.0);
       }
       finalColor = float4(thermalColor, 1.0);
       }
       return finalColor;
   }