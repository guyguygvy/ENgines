...
LRESULT ENgines::Graphics::GraphicsSystem::GraphicsSystemMessageHandler(HWND handle, UINT message, WPARAM wParam, LPARAM lParam)
{
...
void GraphicsSystem::Resize(uint32_t width, uint32_t height)
{
	...
	if (width != GetBackBufferWidth() || height != GetBackBufferHeight()) {
		...
		ASSERT(SUCCEEDED(hr), "GraphicsSystem: failed to access sway chain");
