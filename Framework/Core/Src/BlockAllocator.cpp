...
BlockAllocator::BlockAllocator(const char* name, size_t blockSize, size_t capacity): mName(name), mBlockSize(blockSize), mCapacity(capacity), mFreeBlocks(capacity)
{
	ASSERT(blockSize > 0, "BlockAllocatoer: %s invalid block size", mName.c_str());
	ASSERT(capacity > 0, "BlockAllocatoer: %s invalid block size", mName.c_str());
...
{
	ASSERT(mBlocksAllocatedTotal == mBlocksFreed, "BlockAllocator: %s bit all blocks are freed", mName.c_str);
	std::free(mData);
	mData = nullptr;
...
void* BlockAllocator::Allocate()
{
	...
	{
		LOG("%s no free blobks available", mName.c_str());
		return nullptr;
	}
	...
	LOG("%s allocated blobks at %p, Allocated: %zu, Highest %zu", mName.c_str(), freeBlock, mBlocksAllocatedCurrent, mBlocksHighest);
